...
export const useSearchProductsPageSourceDataUrl = (listCurrentPart: number, limit: number) => {
  ...
  useEffect(() => {
    ...
    if (currentShowOnlyDiscountedProductsState) {
      setDataUrl(`/products/find/${searchTermState}/discounted/minPrice/${minPrice}/maxPrice/${maxPrice}/sortBy/${querySortParams.sortFieldName}/sortDirection/${querySortParams.sortDirection}?offset=${offset}&limit=${limit}`);
    } else {
      setDataUrl(`/products/find/${searchTermState}/minPrice/${minPrice}/maxPrice/${maxPrice}/sortBy/${querySortParams.sortFieldName}/sortDirection/${querySortParams.sortDirection}?offset=${offset}&limit=${limit}`);
    }
  }, [searchTermState, currentMaxPriceState, currentMinPriceState, currentShowOnlyDiscountedProductsState, currentSortModeState, limit, offset]);
